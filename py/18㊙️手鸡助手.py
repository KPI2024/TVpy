...
# -*- coding: utf-8 -*-
"""
手鸡助手 - 四壳通用Python Spider
站点：https://rsk.sjzs3.motorcycles/sjzs/
结构：苹果CMS
"""
import re
...
class Spider(Spider):
    def init(self, extend=""):
        self.siteUrl = "https://rsk.sjzs3.motorcycles"
    ...
    def homeContent(self, filter):
        ...
        classes = [
            ...
            {"type_name": "日韩AV", "type_id": "21"},
            {"type_name": "欧美大片", "type_id": "22"},
            {"type_name": "动漫卡通", "type_id": "23"},
            {"type_name": "三级伦理", "type_id": "24"},
            {"type_name": "成人直播", "type_id": "25"},
            {"type_name": "在线播放", "type_id": "26"},
            {"type_name": "下载资源", "type_id": "27"},
            {"type_name": "图文专区", "type_id": "28"},
        ]
        ...
        # 匹配每个视频条目：链接 + 图片 + 标题
        items = re.findall(
            r'<a[^>]*href="/cn/home/web/index\.php/vod/play/id/(\d+)/sid/1/nid/1\.html"[^>]*>.*?<img[^>]*data-original="([^"]*)"[^>]*alt="([^"]*)"',
            html, re.S
        )
        
        for vid, img, title in items[:20]:
            videos.append({
                "vod_id": vid,
                "vod_name": title,
                "vod_pic": img,
