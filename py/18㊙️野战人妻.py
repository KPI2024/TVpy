# -*- coding: utf-8 -*-
"""
野战人妻 - 四壳通用Python Spider
站点: https://daq.yzrq2.mom
"""
...
class Spider(Spider):
    domain = "https://daq.yzrq2.mom"
    siteName = "野战人妻"
    CATEGORIES = [("20","自拍视频"),("21","强奸乱伦"),("22","无码视频"),("23","有码视频"),("24","人妻熟女"),("25","制服诱惑"),("26","口交颜射"),("27","SM重味"),("28","日韩视频"),("29","欧美视频")]
    ...
    def _parse_list(self, html):
        ...
        # up-list list-img结构
        items = re.findall(r'<ul class="up-list list-img">(.*?)</ul>', html, re.S)
        if items:
            ul_html = items[0]
            lis = re.findall(r'<li>(.*?)</li>', ul_html, re.S)
            for li in lis:
                m = re.search(r'href="/cn/home/web/index.php/vod/play/id/(\d+)/sid/\d+/nid/\d+\.html"', li)
                if not m: continue
                vid = m.group(1)
                ...
                # 标题：从alt属性提取
                name = ""
                t = re.search(r'alt="([^"]+)"', li)
                if t:
                    name = t.group(1).strip()
                    # 清理alt里的多余文字
                    name = re.sub(r'title.*$', '', name).strip()
                if not name or len(name) < 3: continue
                
                # 封面：从img.lazy的data-original提取
                pic = ""
                p = re.search(r'<img[^>]*data-original="([^"]+\.(?:jpg|jpeg|png))"', li)
                if p: pic = p.group(1)
